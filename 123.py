...
if __name__ == '__main__':
    ...
    group.add_argument('-just-dc', action='store_true', default=True,
    ...
    if len(sys.argv)==1:
        print('usage: domain/username password|hash DCIP targetUser')
        # parser.print_help()
        sys.exit(1)
    ...
    domain = sys.argv[1].split("/", 1)[0]
    username = sys.argv[1].split("/", 1)[1]
    password=""
    options.hashes=None
    if sys.argv[2].split(":",1)[0]=="" and len(sys.argv[2].split(":",1))==2:
        options.hashes=sys.argv[2]
    else:
        password = sys.argv[2]
    remoteName=sys.argv[3]
    options.just_dc_user=sys.argv[4]
